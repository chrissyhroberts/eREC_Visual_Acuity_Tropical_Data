  ...
  mutate(
    person_location_cluster =
      case_when(
                ...
                id == "UUID:A81DEA2D-847E-4659-9D9C-301DD61E728C"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:C07D889F-05AB-4257-94CA-EFCDD99F3784"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:4E6B832A-E121-47D1-88F0-77FA99504A19"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:C9F183E1-A19A-43BD-9D8C-BD3D2A74E26C"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:29CF5624-5EF1-4B5E-AA35-983CAC527757"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:E2CB0CE6-3A4D-47A0-B0C1-8B4EA8F5688E"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:06BC364A-DBDD-461F-984F-369A2A1417A8"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:38458415-6993-48C0-976B-CF4C420B82E9"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:E2CB0CE6-3A4D-47A0-B0C1-8B4EA8F5688E"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:E73BF45F-E148-4BD1-AD0C-843E7BF21390"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:587E4458-49E4-473D-9B60-A8D2BD9659BC"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:056E9DFA-34D8-4903-8C1B-9C16A6DDD4EA"~"045_TOSARAWAN_MALAMA_ARKI",
                id == "UUID:84730F3D-A7D1-4546-A65D-00FB5695FDB0"~"045_TOSARAWAN_MALAMA_ARKI",
...
ggplotly(ggplot(filter(filtered_df,survey_duration<3600),aes(sex,survey_duration,fill=sex))+
  ...
  ggtitle("Survey Duration by Age and Sex (Nigeria)")
)
...
```{r}
filtered_df<-df %>% 
  filter(person_location_country=="NIGERIA") %>% mutate(
...
ggplotly(ggplot(filter(filtered_df,survey_duration<3600),aes(sex,survey_duration,fill=education_level))+
  ...
  ggtitle("Survey Duration by Education and Sex (Nigeria)")
)
...
ggplotly(ggplot(filter(filtered_df,survey_duration<3600),aes(age_group,survey_duration,fill=education_level))+
  geom_boxplot()+
  facet_grid(.~education_level) +
  ggtitle("Survey Duration by Education and Age (Nigeria)")+
...
ggplotly(ggplot(filter(filtered_df,survey_duration<3600),aes(person_location_standard_minimal_peek,survey_duration,fill=person_location_standard_minimal_peek))+
  geom_boxplot()+
  facet_grid(.~age_group) +
  ggtitle("Survey Duration by Age and Protocol (Nigeria)")
)
...
# Display a message about the number of referrals by each method

bilateral_logMAR_Peek <- df %>% 
  filter(person_location_standard_minimal_peek=="PEEK") %>% 
  select(id,person_location_standard_minimal_peek,peek_visual_acuity_tests_peek_uncorrected_right_eye_log_mar,peek_visual_acuity_tests_peek_uncorrected_left_eye_log_mar) %>%
  mutate(
    peek_visual_acuity_tests_peek_uncorrected_right_eye_log_mar = as.numeric(as.character(peek_visual_acuity_tests_peek_uncorrected_right_eye_log_mar)),
    peek_visual_acuity_tests_peek_uncorrected_left_eye_log_mar =  as.numeric(as.character(peek_visual_acuity_tests_peek_uncorrected_left_eye_log_mar)), 
    bilateral_fail_uncorrected = case_when(
      
      peek_visual_acuity_tests_peek_uncorrected_right_eye_log_mar>=0.3 & peek_visual_acuity_tests_peek_uncorrected_left_eye_log_mar >=0.3 ~ "BILATERAL_FAIL_0.3",
    peek_visual_acuity_tests_peek_uncorrected_right_eye_log_mar<0.3 | peek_visual_acuity_tests_peek_uncorrected_left_eye_log_mar <0.3 ~ "PASS")
    )

  
bilateral_logMAR_Peekminimal <- df %>% 
  filter(person_location_standard_minimal_peek=="PEEK_MINIMAL") %>% 
  select(id,person_location_standard_minimal_peek,peek_visual_acuity_tests_rapid_peek_uncorrected_right_eye_rapid_pass,peek_visual_acuity_tests_rapid_peek_uncorrected_left_eye_rapid_pass) %>%
  mutate(
    bilateral_fail_uncorrected = case_when(
      peek_visual_acuity_tests_rapid_peek_uncorrected_right_eye_rapid_pass == FALSE & peek_visual_acuity_tests_rapid_peek_uncorrected_left_eye_rapid_pass == FALSE ~ "BILATERAL_FAIL_0.3",
      
       peek_visual_acuity_tests_rapid_peek_uncorrected_right_eye_rapid_pass == "UNDEFINED" & peek_visual_acuity_tests_rapid_peek_uncorrected_left_eye_rapid_pass == "UNDEFINED" ~ "BILATERAL_FAIL_0.3",
      peek_visual_acuity_tests_rapid_peek_uncorrected_right_eye_rapid_pass == TRUE | peek_visual_acuity_tests_rapid_peek_uncorrected_left_eye_rapid_pass == TRUE ~ "PASS"
    )
        
          )

bilateral_logMAR_combined <- bind_rows(bilateral_logMAR_Peek,bilateral_logMAR_Peekminimal)


# Function to calculate confidence interval
conf_interval <- function(prop, n, confidence = 0.95) {
  error <- qnorm((1 + confidence) / 2) * sqrt((prop * (1 - prop)) / n)
  return(error)
}

bilateral_logMAR_combined_results <- bilateral_logMAR_combined %>% 
  group_by(person_location_standard_minimal_peek) %>% 
  mutate(denom = n()) %>% 
  group_by(person_location_standard_minimal_peek, bilateral_fail_uncorrected) %>% 
  reframe(
    n = n(),
    denom = first(denom),
    prop = n / first(denom),
    error = conf_interval(prop, n),
    ci_lower = prop - error,
    ci_upper = prop + error
  ) %>% 
  distinct()
...
ggplotly(
ggplot(filter(bilateral_logMAR_combined_results,bilateral_fail_uncorrected=="BILATERAL_FAIL_0.3"),aes (person_location_standard_minimal_peek,prop,fill=bilateral_fail_uncorrected))+
  geom_bar(stat="identity",position="dodge")+
geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), 
                position = position_dodge(width = 0.9), 
                width = 0.25) +
  labs(
    x = "Person Location Standard Minimal Peek",
    y = "Proportion",
    fill = "Bilateral Fail Uncorrected"
  ) +
  theme_minimal() +
  theme(
  ...
  ggtitle("DV by Person Location and Bilateral LogMAR >=0.3")+
  ylim(0,1)
)
```
...
```{r}
bilateral_logMAR_combined<-bilateral_logMAR_combined %>% filter(!is.na(bilateral_fail_uncorrected))
#summary(as.factor(bilateral_logMAR_combined$bilateral_fail_uncorrected))


# Display a message about the number of referrals by each method
message("Number of bilateral DV fails (LogMAR>=0.3) by each method")

# Create a new column 'referred' based on the presence of 'referral_name'

# Create a contingency table
referral_location_table <- table(bilateral_logMAR_combined$bilateral_fail_uncorrected, bilateral_logMAR_combined$person_location_standard_minimal_peek)

# Print the contingency table
print(referral_location_table)

# Perform the chi-squared test
chi_squared_result <- chisq.test(referral_location_table)
...
bilateral_logMAR_Peek <- df %>% 
  filter(person_location_standard_minimal_peek=="PEEK",
         age >= 35) %>% 
  select(id,person_location_standard_minimal_peek,peek_visual_acuity_tests_peek_nva_uncorrected_right_eye_log_mar,peek_visual_acuity_tests_peek_nva_uncorrected_left_eye_log_mar) %>%
  mutate(
    peek_visual_acuity_tests_peek_nva_uncorrected_right_eye_log_mar = as.numeric(as.character(peek_visual_acuity_tests_peek_nva_uncorrected_right_eye_log_mar)),
    peek_visual_acuity_tests_peek_nva_uncorrected_left_eye_log_mar =  as.numeric(as.character(peek_visual_acuity_tests_peek_nva_uncorrected_left_eye_log_mar)), 
    bilateral_fail_uncorrected = case_when(
      
      peek_visual_acuity_tests_peek_nva_uncorrected_right_eye_log_mar>=0.3 & peek_visual_acuity_tests_peek_nva_uncorrected_left_eye_log_mar >=0.3 ~ "BILATERAL_FAIL_0.3",
    peek_visual_acuity_tests_peek_nva_uncorrected_right_eye_log_mar<0.3 | peek_visual_acuity_tests_peek_nva_uncorrected_left_eye_log_mar <0.3 ~ "PASS")
    )
...
geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), 
  ...
  ggtitle("NV by Person Location and Bilateral LogMAR >=0.3")+
  ylim(0,1)
)
```
...
# Display a message about the number of referrals by each method
message("Number of bilateral NV fails (LogMAR>=0.3) by each method")
...
# Create a new column 'referred' based on the presence of 'referral_name'
referrals <- df %>% 
  mutate(referred = ifelse(is.na(referral_name), "NOT_REFERRED", "REFERRED")) %>% 
  group_by(person_location_standard_minimal_peek) %>% 
  mutate(denom = n()) %>% 
  group_by(person_location_standard_minimal_peek, referred) %>% 
  reframe(
    n = n(),
    denom = first(denom),
    prop = n / first(denom),
...
ggplotly(
ggplot(filter(referrals,referred=="REFERRED"),aes (person_location_standard_minimal_peek,prop,fill=referred))+
  geom_bar(stat="identity",position="dodge")+
geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), 
                position = position_dodge(width = 0.9), 
                width = 0.25) +
...
# Display a message about the number of referrals by each method
message("Number of referrals by each method")

df<- df %>%   mutate(referred = ifelse(is.na(referral_name), "NOT_REFERRED", "REFERRED"))
# Create a contingency table
referral_location_table <- table(df$referred, df$person_location_standard_minimal_peek)
...
logmar_age_left<-df %>% 
  filter(person_location_country=="NIGERIA",
         person_location_standard_minimal_peek=="PEEK") %>% 
  mutate(age_group = cut(age,breaks = c(0,10,15,20,30,40,50,60,120),labels = c("0_10","11_15","16_20","21_30","31_40","41_50","51_60","61_120")),
         logmar= as.numeric(as.character(peek_visual_acuity_tests_peek_uncorrected_left_eye_log_mar)))

logmar_age_right<-df %>% 
  filter(person_location_country=="NIGERIA",
         person_location_standard_minimal_peek=="PEEK") %>% 
  mutate(age_group = cut(age,breaks = c(0,10,15,20,30,40,50,60,120),labels = c("0_10","11_15","16_20","21_30","31_40","41_50","51_60","61_120")),
         logmar = as.numeric(as.character(peek_visual_acuity_tests_peek_uncorrected_right_eye_log_mar)))

logmar_age<-bind_rows(logmar_age_left,logmar_age_right)

ggplot(logmar_age,aes(age_group,logmar))+
  geom_boxplot()+
  ggtitle("LogMAR by Age (Nigeria)")+
  geom_hline(yintercept = 0.3,lty=2,col="red")
