...
p <- plot_ly(data = daily_counts, 
  ...
  layout(barmode = 'stack',
         title = "Daily Submissions by Cluster (Nigeria)",
...
# Create a leaflet map with clustering and initial zoom
...
leaflet(df) %>%
  addTiles(options = tileOptions(maxZoom = 8)) %>%  # Set the maximum zoom level
  addCircleMarkers(
...
p <- plot_ly(data = daily_counts, 
             ...
             type = 'bar') %>% 
  layout(barmode = 'stack',
         title = "Daily Submissions by Cluster (Ethiopia)",
         xaxis = list(title = 'Date',
                      tickangle = -90,
                      tickformat = "%Y-%m-%d",
                      tickmode = 'linear',
