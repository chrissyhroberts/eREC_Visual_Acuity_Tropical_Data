...
# Round latitude and longitude to 3 decimal places (~100 m accuracy)
df <- df %>%
  mutate(
    rounded_longitude = round(location_geolocation_longitude, 3),
    rounded_latitude = round(location_geolocation_latitude, 3)
  )

# Define a color palette for the clusters
palette <- colorFactor(palette = brewer.pal(9, "Set1"), domain = df$person_location_cluster)

# Create the leaflet map using the rounded coordinates
leaflet(df) %>%
  addTiles(options = tileOptions(maxZoom = 13)) %>%  # Set the maximum zoom level
  addCircleMarkers(
    lng = ~rounded_longitude,
    lat = ~rounded_latitude,
    radius = ~location_geolocation_accuracy / 10,  # Adjust radius based on accuracy
    color = ~palette(person_location_cluster),
    fillOpacity = 0.5,
    popup = ~paste("Cluster ID:", person_location_cluster),  # Add popup with cluster ID
    clusterOptions = markerClusterOptions(
      spiderfyOnMaxZoom = FALSE,  # Disable radial expansion
      showCoverageOnHover = TRUE,  # Show coverage area on hover
      zoomToBoundsOnClick = TRUE,  # Zoom to bounds when clicked
      maxClusterRadius = 40  # Radius in pixels to cluster
    )
  )
